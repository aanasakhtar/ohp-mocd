"""
data/load_amazon.py — Download and load SNAP Amazon co-purchasing network.
...
from config import AMAZON_CONFIG
...
def _resolve_working_url(candidates: list[str]) -> str:
    ...
    if last_error is None:
        raise RuntimeError("No Amazon URL candidates provided")
    raise last_error
...
def load_amazon(cfg: dict = AMAZON_CONFIG) -> tuple[nx.Graph, list[frozenset]]:
    save_dir = Path(cfg["save_dir"])
    save_dir.mkdir(parents=True, exist_ok=True)

    graph_gz = save_dir / "com-amazon.ungraph.txt.gz"
    cmty_gz = save_dir / "com-amazon.all.dedup.cmty.txt.gz"
    ...
    print("Parsing Amazon edge list ...")
    G = nx.Graph()
    ...
    print("Parsing Amazon community file ...")
...
if __name__ == "__main__":
    G, cmty = load_amazon()
    print(f"Ready: Amazon graph has {G.number_of_nodes()} nodes, {len(cmty)} communities.")
