...
### 2.3. Stochastic Genetic Operators
- **`mod::tests::crossover_secondary_consensus`**
  - **Objective:** Tests consensus ensemble crossover over 4 parents.
  - **Checks:** Secondary memberships are kept when held by $\ge 50\%$ of parents.
- **`mod::tests::mutation_local_move_majority`**
  - **Objective:** Validates local-move majority mutation rules.
  - **Checks:** Node adopts neighborhood majority community and runner-up overlaps.
